...
class BodyParams extends Factory {
  constructor(args) {
    super(args);
    ...
    try {
      ...
      if (args["$create"]) {
        ...
        const changeFunction = oldData => {
          oldData.data[this.bodyValues.collection.link] = newUserBodyValues;
          return oldData;
        };
    ...
    try {
      return await Object.keys(args).reduce(async (acc, key) => {
        ...
        if (key === "$create" && args[key]["values"]) {
          const value = await this.bodyValues._create(args[key]["values"]);
        ...
        if (
          ...
          const value = await this.bodyValues._deleteById(
            args[key]["values"]["id"]
          );
