...
class Employees extends Factory {
  constructor(args) {
    ...
    const {
      items: { bodyParams, positions },
    } = args;
    if (!bodyParams) throw new Error("bodyParams instance required");
    if (!positions) throw new Error("positions instance required");
  ...
  async _expand(item) {
    try {
      const expandedPositions = await this.positions._readMany(item.positions);
      ...
      const allParams = await this.bodyParams._readMany(data.params);
  ...
  async _expandList(items) {
    try {
      ...
      const expandedPositions = await this.positions._readMany(positionsList);
      ...
      const _args = await Object.keys(args).reduce(
        async (acc, key) => {
          ...
          if (key === "$push" && args[key]["bodyParams"]) {
            ...
            const param = await this.bodyParams._readById(paramId);
